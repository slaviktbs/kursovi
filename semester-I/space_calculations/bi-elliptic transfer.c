...
int main(void)
{
    double mu, v_par, a_par, v_tr_per_1, v_tr_per_2, v_tr_apo_1, v_tr_apo_2, v_tar, a_tar, a_tr_1, a__tr_2, delta_v_1, delta_v_2, delta_v_3, f;
    ...
    v_tr_per_1 = sqrt(mu*(2/a_par-1/a_tr_1));   // !! ?? ПРОВЕРИТЬ ?? !!

    delta_v_1 = v_tr_per_1 - v_par;

    v_tr_apo_1 = sqrt(mu*(2/a_tar-1/a_tr_1));   // !! ?? ПРОВЕРИТЬ ?? !!    ИЛИ 2/(2*a_tr_1-a_par) ?? !!

    a__tr_2 =  (a_tar - a_par + 2*a_tr_1)/2;  
    
    v_tr_apo_2 = sqrt(mu*(2/a_tar-1/a__tr_2));   // !! ?? ПРОВЕРИТЬ ?? !!

    delta_v_2 = v_tr_apo_2 - v_tr_apo_1;

    v_tr_per_2 = sqrt(mu*(2/a_par-1/a__tr_2));   // !! ?? ПРОВЕРИТЬ ?? !!
